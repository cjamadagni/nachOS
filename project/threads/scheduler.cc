    	...
    	case SCHED_MLQ:
			{	//YOUR PROJECT2 CODE HERE
				if(thread->getPriority() == 3)
					MultiLevelList[2].Append(thread);
				else if(thread->getPriority() == 2)
					MultiLevelList[1].Append(thread); 
...
Scheduler::FindNextToRun ()
{
   ...
   switch(policy)
   {
   	...
   	{
		//YOUR PROJECT2 CODE HERE
		if(MultiLevelList[2].IsEmpty() == false)
			next_to_run = MultiLevelList[2].Remove();
		else if(MultiLevelList[1].IsEmpty() == false)
   	...
   	case SCHED_PRIO_P:
   	{
		//YOUR PROJECT2 CODE HERE
		int add;
		next_to_run = readyList->SortedRemove(&add);	
